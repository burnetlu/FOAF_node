...
D3_BubbleRepresentation.load = function (json) {
    ...
    var circle = container.selectAll("circle")
            ...
            .attr("class", function (d) {
                return d.parent ? d.children ? "node" : "node node--leaf" : "node node--treeJson";
            })
            ...
            .on("click", function (d) {
                ...
                if (focus !== d)
                    zoomfonc(d), d3.event.stopPropagation();
                ...
                else if (sansEspace.test(d.children[0].name.toString()) == false)
                    d3_utils.show_wikipedia(d.children[0].name);
    ...
    d3.selectAll('.rotate').on('click', function () {
        console.log('inactif');
    });
    d3.selectAll('.dragAndDrop').on('click', function () {
        console.log('inactif');
    });
